/* ===== Білім Аралы — игра "Устройства компьютера" =====
   Цифровая грамотность, 1 класс, по программе (приложение 27,
   цель 1.1.1.2): "называть основные устройства компьютера".
   8 устройств: монитор, клавиатура, мышь, принтер, колонки,
   ноутбук, наушники, джойстик — расширено с 6 для разнообразия
   при повторных кругах.
   Используются только давно поддерживаемые эмодзи (2010-2016).
   Сессия = несколько кругов подряд, минимум 15 вопросов.
   Использование: initComputerDevicesGame('game-root') после
   загрузки DOM.
*/
function initComputerDevicesGame(containerId){
  ...
  function renderFinish(){
    recordGameResult('computerdevices_kz', score, TOTAL);
    var m = msgs();
    root.innerHTML =
