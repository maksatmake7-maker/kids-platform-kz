/* ===== Білім Аралы — игра "Что нужно ребёнку?" (v2 — режим освоения) =====
   Естествознание, 1 класс, по программе (приложение 28, цель 1.2.3.3):
   "определять потребности человека, необходимые для его роста и
   развития" — базовое содержание конкретно называет: питание, отдых.
   Показываем предмет, ребёнок определяет — нужно это для роста и
   здоровья, или нет. Предметов всего 10 — они не заканчиваются, а
   случайно повторяются (без повтора одного и того же предмета два
   раза подряд); чтобы "освоить" тему, нужно ответить верно 10 раз
   ПОДРЯД. Ошибка сбрасывает серию, но не прерывает игру.
   Использование: initHumanNeedsGame('game-root') — humanneeds.html не трогать.
*/
function initHumanNeedsGame(containerId){
  ...
  function renderFinish(){
    recordGameResult('humanneeds', STREAK_NEEDED, STREAK_NEEDED);
    var m = msgs();
    root.innerHTML =
